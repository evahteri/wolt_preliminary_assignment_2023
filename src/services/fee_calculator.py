...
class FeeCalculator:
    ...
    def __init__(self, response_object):
        ...
        self.fee = 0
        self.rush_start_time = datetime.datetime.strptime(
            "15:00:00", "%H:%M:%S")
    ...
    def _delivery_distance_fee(self):
        ...
        if self.response_object.delivery_distance > 1000:  # Counting the delivery fee for the extra 500 metres
            # Extra charge is counted by deducting the first 1000 metres from the distance, then dividing it with 500 and rounding it upwards.
            extra_charge = math.ceil(
                (self.response_object.delivery_distance - 1000) / 500)
            # Add the extra charge and multiple it with 100 so that is matches the notation.
            self.fee += extra_charge * 100
    ...
    def _number_of_items(self):
        ...
        if self.response_object.number_of_items > 4:
            ...
            extra_items = self.response_object.number_of_items - 4
            # Multiply the amount by 50 (50 cents) and add it to the total fee.
            self.fee += extra_items * 50
        # If there are more than 12 items, add the bulk fee (120 cents).
        if self.response_object.number_of_items > 12:
            self.fee += 120
